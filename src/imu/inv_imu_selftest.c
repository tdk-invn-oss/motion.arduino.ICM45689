...
static int selftest_edmp_configure(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	fifo_sram_sleep_t fifo_sram_sleep;
	uint8_t           value;
	uint16_t          start_addr = RAM_SELFTEST_IMG_PRGM_BASE;
	/* Only 8 MSB of SP address is written to register */
	uint8_t        stack_addr = (uint8_t)(RAM_SELFTEST_IMG_STACK_END >> 8);
	static uint8_t ram_img[]  = {
#include "imu/edmp_prgm_ram_selftest.h"
	};

	/* Set Start address for edmp IRQ 2 handler */
	status |= inv_imu_write_reg(s, EDMP_PRGRM_IRQ2_0, sizeof(start_addr), (uint8_t *)&start_addr);
	...
	/* Clear SRAM data area */
	value = 0;
	for (int i = 0; i < RAM_SELFTEST_IMG_PRGM_BASE; i++)
		status |= inv_imu_write_sram(s, (uint32_t)EDMP_RAM_BASE + i, 1, &value);

	/* Load code in SRAM */
	status |= inv_imu_write_sram(s, RAM_SELFTEST_IMG_PRGM_BASE, sizeof(ram_img), ram_img);
                     ...
                     inv_imu_selftest_output_t *st_output)
{
	...
	/* Configure start addresses and load selftest RAM image as we reset the device */
	rc |= selftest_edmp_configure(s);
